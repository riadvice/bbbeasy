...
{
    final protected const COLLECT_SETTINGS_ROUTE = 'GET /collect-settings';
    ...
    {
        $test = $this->newTest();
        $f3->mock(self::COLLECT_SETTINGS_ROUTE);

        json_decode($f3->get('RESPONSE'));
        $test->expect(JSON_ERROR_NONE === json_last_error(), 'Collect settings');
