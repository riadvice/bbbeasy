...
class Statera
{
    ...
    private static function isCoverageEnabled()
    {
        if (self::$coverageEnabled === null) {
            self::$coverageEnabled = array_key_exists('statera', $_GET) && $_GET['statera'] === 'withCoverage';
    ...
    public static function writeCoverageResult(): void
    {
        if (self::isCoverageEnabled()) {
            ...
            $publicFolder = dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'statera';
            $writer       = new Facade();
